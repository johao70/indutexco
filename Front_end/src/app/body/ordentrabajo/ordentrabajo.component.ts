...
export class OrdentrabajoComponent implements OnInit {
  ...
  formularioCliente() {
    this.clienteForm = this.fb.group({
      identificacion: [
        "",
        [Validators.required, Validators.pattern("([0|1|2]{1})([0-9]{9})")],
      ],
      nombres: [
        "",
        [Validators.required, Validators.pattern("[A-Za-z]{1}[a-z]{3,30}")],
      ],
      apellidos: [
        "",
        [Validators.required, Validators.pattern("[A-Za-z]{1}[a-zñ]{3,30}")],
      ],
      telefonos: [
        "",
        [
          Validators.required,
          Validators.pattern("(((09)|(08)|(06)){1})([0-9]{8})"),
        ],
      ],
  ...
  nuevafecha = new Date();
  fecha_orden =
    this.nuevafecha.getDate() +
    "/" +
    (this.nuevafecha.getMonth() + 1) +
    "/" +
    this.nuevafecha.getFullYear();
  idclientes: number;
  ...
  getDataTable = () => {
    let tabla = "F_ordenes";
    this.http.get<any>(environment.API_URL + `${tabla}`).subscribe((data) => {
  ...
  postDataTable = () => {
    ...
    this.http.post(environment.API_URL, register).subscribe((data) => {
      // this.postData = data
    });
  ...
  deleteDataTable = (value) => {
    let tabla = "ordenes";
    this.http
      .delete(environment.API_URL + `?tabla=${tabla}&&id=${value}`)
      .subscribe((data) => {});
    window.location.reload();
  };
  ...
  getDatabyID = (value) => {
    let tabla = "ordenes";
    this.http
      .get<any>(environment.API_URL + `byid?tabla=${tabla}&&id=${value}`)
      .subscribe((data) => {
  ...
  postDataDetalleOrden = () => {
    ...
    let f_cantidadEtiquetas = this.detalleOrdenForm.get("f_cantidadEtiquetas")
      .value;
    let f_botones = this.detalleOrdenForm.get("f_botones").value;
    let f_cantidadBotones = this.detalleOrdenForm.get("f_cantidadBotones")
      .value;
  ...
  getPDF = () => {
    let ruta = "pdf";
    this.http.get<any>(environment.API_URL + `${ruta}`).subscribe((data) => {
